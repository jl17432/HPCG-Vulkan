...
namespace
{
	...
	BakedModel load_baked_model_( FILE* aFin, char const* aInputName )
	{
		...
		for( std::uint32_t i = 0; i < materialCount; ++i )
		{
			...
			checked_read_( aFin, sizeof(float)*3, &info.baseColor.x );
			checked_read_( aFin, sizeof(float)*3, &info.emissiveColor.x );
			checked_read_( aFin, sizeof(float), &info.roughness );
			checked_read_( aFin, sizeof(float), &info.metalness );
