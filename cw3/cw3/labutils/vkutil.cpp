...
namespace labutils
{
	ShaderModule load_shader_module(VulkanContext const& aContext, char const* aSpirvPath)
	{
		...
		if (std::FILE* fin = std::fopen(aSpirvPath, "rb"))
		{
			...
			if (auto const res = vkCreateShaderModule(aContext.device, &moduleInfo, nullptr,
				&smod); VK_SUCCESS != res)
			{
	...
	ImageView create_dummy_image_view_texture2d(VulkanContext const& aContext, VkImage aImage)
	{
		VkImageViewCreateInfo viewInfo = {};
		viewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
		viewInfo.image = aImage;
