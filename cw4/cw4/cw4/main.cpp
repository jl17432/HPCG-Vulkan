...
#		define SHADERDIR_ "assets/cw4/shaders/"
		...
		constexpr char const* v_shadow_ShaderPath = SHADERDIR_ "shadow.vert.spv";
		constexpr char const* f_shadow_ShaderPath = SHADERDIR_ "shadow.frag.spv";
#		undef SHADERDIR_
...
#		undef ASSERTDIR_
		...
		constexpr int shadow_view_width = 1024;
		constexpr int shadow_view_height = 1024;
	...
	struct UserState
	{
		...
		glm::vec3 lightPosWorld = glm::vec3(0.f, 5.f, -1.f);
	...
	////////////////////////////////////
	//    for shadow mapping pass     //
	////////////////////////////////////

	lut::RenderPass create_shadow_pass(lut::VulkanWindow const& aWindow);

	lut::DescriptorSetLayout create_shadow_layout(lut::VulkanWindow const& aWindow);

	lut::PipelineLayout create_shadow_pipeline_layout(lut::VulkanContext const& aContext, VkDescriptorSetLayout const& aSceneLayout, VkDescriptorSetLayout const& aObjectLayout);
	...
	std::tuple<lut::Image, lut::ImageView> create_shadow_depth_buffer(lut::VulkanWindow const& aWindow, lut::Allocator const& aAllocator);
}
...
int main() try
{
	...
	lut::RenderPass shadowPass = create_shadow_pass(window);
	...
	lut::PipelineLayout shadowPipeLayout = create_shadow_pipeline_layout(window, sceneLayout.handle, shadowLayout.handle);
	...
	lut::Pipeline shadowPipe = create_shadow_pipeline(window, shadowPass.handle, shadowPipeLayout.handle);

	auto [depthBuffer, depthBufferView] = create_depth_buffer(window, allocator);
	auto [shadowBuffer, shadowBufferView] = create_shadow_depth_buffer(window, allocator);
	...
	lut::Framebuffer shadow_framebuffer;
	create_shadow_framebuffer(window, shadowPass.handle, shadow_framebuffer, shadowBufferView.handle);
	...
	// create dummy image for normal map
	lut::Image dummyImage = lut::create_image_texture2d(allocator, window.swapchainExtent.width, window.swapchainExtent.height, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_USAGE_SAMPLED_BIT |
		VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT);
	lut::ImageView dummyImageView = lut::create_image_view_texture2d(window, dummyImage.image, VK_FORMAT_R8G8B8A8_SRGB);
	...
	for (auto i = 0; i < bakedfragments.size(); i++)
	{
		...
		auto normalMapID = bakedfragments[i].normalMaskTextureId;
		...
		if (normalMapID > imageViewVec.size() || normalMapID == -1 || !normalMapID)
		{
			textureInfo[3].imageView = dummyImageView.handle;
		}
		else
		{
			textureInfo[3].imageView = imageViewVec[normalMapID].handle;
		}
		textureInfo[3].sampler = defaultSampler.handle;

		textureInfo[4].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		textureInfo[4].imageView = shadowBufferView.handle;
		textureInfo[4].sampler = shadowSampler.handle;
	...
	for (auto i = 0; i < alphafragments.size(); i++)
	{
		...
		auto normalMapID = alphafragments[i].normalMaskTextureId;
		...
		if (normalMapID > imageViewVec.size() || normalMapID == -1 || !normalMapID)
		{
			textureInfo[4].imageView = dummyImageView.handle;
		}
		else
		{
			textureInfo[4].imageView = imageViewVec[normalMapID].handle;
		}
		textureInfo[4].sampler = defaultSampler.handle;

		textureInfo[5].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		textureInfo[5].imageView = shadowBufferView.handle;
		textureInfo[5].sampler = shadowSampler.handle;
	...
	while (!glfwWindowShouldClose(window.window))
	{
		...
		if (recreateSwapchain || state.changeMode)
		{
			...
			if (changes.changedFormat)
			{
				...
				shadowPass = create_shadow_pass(window);
			}
			if (changes.changedSize)
			{
				std::tie(depthBuffer, depthBufferView) = create_depth_buffer(window, allocator);
				std::tie(shadowBuffer, shadowBufferView) = create_shadow_depth_buffer(window, allocator);
			...
			if (changes.changedSize || state.changeMode)
			{
				...
				shadowPipe = create_shadow_pipeline(window, shadowPass.handle, shadowPipeLayout.handle);
...
namespace
{
	...
	void update_user_state(UserState& aState, float aElapsedTime)
	{
		...
		if (aState.inputMap[std::size_t(EInputState::lightDown)])
			light = glm::translate(glm::vec3(0.f, -lightMove, 0.f)) * glm::vec4(light, 1.f);
		//std::cout << "light position: " << light.x << " " << light.y << " " << light.z << "\n";
	}
...
namespace
{
	void update_scene_uniforms(glsl::SceneUniform& aSceneUniforms, std::uint32_t aFramebufferWidth, std::uint32_t aFramebufferHeight, UserState const& aState)
	{
		...
		float const lightAspect = cfg::shadow_view_width / cfg::shadow_view_height;
		...
		aSceneUniforms.lightPos = glm::vec4(aState.lightPosWorld, 1.f);
		

		aSceneUniforms.lightProjection = glm::perspectiveRH_ZO(
		...
		glm::vec3 lightDir = glm::vec3(0.f, -1.f, -1.2f);
		glm::vec3 targetPos = aState.lightPosWorld + lightDir;
		glm::vec3 upVector = glm::vec3(0.0f, 1.0f, 0.0f);
		//glm::mat4 lightViewMat = glm::lookAt(aState.lightPosWorld, targetPos, upVector);
		glm::mat4 lightViewMat = glm::lookAt(aState.lightPosWorld, glm::vec3(0,0,-100), upVector);
		aSceneUniforms.lightViewMat = lightViewMat;

		aSceneUniforms.shadowMapResolution = float(cfg::shadow_view_width / cfg::shadow_view_height);
		aSceneUniforms.swapchainResolution = float(aFramebufferWidth / aFramebufferHeight);
	}
}
...
namespace
{
	...
	lut::DescriptorSetLayout create_alpha_descriptor_layout(lut::VulkanWindow const& aWindow)
	{
		...
		bindings[5].binding = 5; // this must match the shaders 
		...
		)
	{
		...
		shadowClearValues[0].depthStencil.depth = 1.f; // new!

		VkViewport shadowViewport = {};
		shadowViewport.x = 0.0f;
		shadowViewport.y = 0.0f;
		shadowViewport.width = (float)cfg::shadow_view_width;
		shadowViewport.height = (float)cfg::shadow_view_height;
		shadowViewport.minDepth = 0.0f;
		shadowViewport.maxDepth = 1.0f;

		VkRect2D shadowScissor = {};
		shadowScissor.offset = { 0, 0 };
		shadowScissor.extent = VkExtent2D{ cfg::shadow_view_width, cfg::shadow_view_height };
		...
		shadowPassInfo.renderArea.extent = VkExtent2D{ cfg::shadow_view_width , cfg::shadow_view_height };
		...
		vkCmdSetViewport(aCmdBuff, 0, 1, &shadowViewport);
		vkCmdSetScissor(aCmdBuff, 0, 1, &shadowScissor);

		vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, shadowLayout, 0, 1, &aSceneDescriptors, 0, nullptr);
		for (size_t i = 0; i < bakedfragments.size(); i++)
		{
			...
			VkBuffer buffers[1] = { bakedfragments[i].positions.buffer };
		...
		for (size_t i = 0; i < alphafragments.size(); i++)
		{
			...
			VkBuffer buffers[1] = { alphafragments[i].positions.buffer };
			VkDeviceSize offsets[1]{  };
		...
		for (size_t i = 0; i < bakedfragments.size(); i++)
		{
			...
			VkBuffer buffers[4] = { bakedfragments[i].positions.buffer, bakedfragments[i].texcoords.buffer, bakedfragments[i].normals.buffer, bakedfragments[i].tangents.buffer };
		...
		for (size_t i = 0; i < alphafragments.size(); i++)
		{
			...
			VkBuffer buffers[4] = { alphafragments[i].positions.buffer, alphafragments[i].texcoords.buffer, alphafragments[i].normals.buffer, alphafragments[i].tangents.buffer };
			VkDeviceSize offsets[4]{  };
			vkCmdBindVertexBuffers(aCmdBuff, 0, 4, buffers, offsets);
		...
		//barrier2.oldLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;  // the old layout of the image
		//barrier2.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;  // the new layout to which you want to transition the image
		//barrier2.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
		//barrier2.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
		//barrier2.image = shadowImage;  // the image that you want to transition
		...
		//if (VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL == VK_IMAGE_LAYOUT_UNDEFINED && VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL) {
		//	barrier2.srcAccessMask = 0;
		//	barrier2.dstAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
		//	sourceStage2 = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
		//	destinationStage2 = VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
		//}
		//else if (VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL && VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL) {
		//	barrier2.srcAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
		//	barrier2.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
		//	sourceStage2 = VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
		//	destinationStage2 = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;
		//}
		//else {
...
namespace
{
	lut::RenderPass create_shadow_pass(lut::VulkanWindow const& aWindow)
	{
		...
		dep1.dstSubpass = 0;
		dep1.srcStageMask = VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT;
		dep1.dstStageMask = VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
		dep1.srcAccessMask = VK_ACCESS_MEMORY_READ_BIT;
	...
	lut::Pipeline create_shadow_pipeline(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, VkPipelineLayout aPipelineLayout)
	{
		lut::ShaderModule vert = lut::load_shader_module(aWindow, cfg::v_shadow_ShaderPath);
		lut::ShaderModule frag = lut::load_shader_module(aWindow, cfg::f_shadow_ShaderPath);
		...
		viewport.x = 0.f;
		viewport.y = 0.f;
		viewport.width = float(cfg::shadow_view_width);
		viewport.height = float(cfg::shadow_view_height);
		viewport.minDepth = 0.f;
		viewport.maxDepth = 1.f;

		VkRect2D scissor{};
		scissor.offset = VkOffset2D{ 0, 0 };
		scissor.extent = VkExtent2D{ cfg::shadow_view_width, cfg::shadow_view_height };
	...
	)
	{
		...
		fbInfo.pAttachments = attachments;
		fbInfo.width = (float)cfg::shadow_view_width;
		fbInfo.height = (float)cfg::shadow_view_height;
		fbInfo.layers = 1;
	...
	std::tuple<lut::Image, lut::ImageView> create_shadow_depth_buffer(lut::VulkanWindow const& aWindow, lut::Allocator const& aAllocator)
	{
		...
		imageInfo.imageType = VK_IMAGE_TYPE_2D;
		imageInfo.format = cfg::kDepthFormat;
		imageInfo.extent.width = (float)cfg::shadow_view_width;
		imageInfo.extent.height = (float)cfg::shadow_view_height;
		imageInfo.extent.depth = 1;
		imageInfo.mipLevels = 1;
		imageInfo.arrayLayers = 1;
		imageInfo.samples = VK_SAMPLE_COUNT_1_BIT;
