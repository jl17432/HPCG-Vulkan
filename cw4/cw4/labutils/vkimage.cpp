...
namespace labutils
{
	Image load_image_texture2d(char const* aPath, VulkanContext const& aContext, VkCommandPool aCmdPool, Allocator const& aAllocator, VkFormat fmt, int channel)
	{
		channel = 4;
		...
		stbi_uc* data = stbi_load(aPath, &baseWidthi, &baseHeighti, &baseChannelsi, channel);  // want 4 c h annel s = RGBA
		...
		auto const sizeInBytes = baseWidth * baseHeight * channel;
		...
		// Create image 
		Image ret = create_image_texture2d(aAllocator, baseWidth, baseHeight, fmt, VK_IMAGE_USAGE_SAMPLED_BIT |
