...
#		endif // ~ debug builds
		...
		ret.window = glfwCreateWindow(1280, 720, "sc22j2l Coursework 01", nullptr, nullptr);
		if (!ret.window)
		{
			char const* errMsg = nullptr;
			glfwGetError(&errMsg);
	...
	SwapChanges recreate_swapchain(VulkanWindow& aWindow)
	{
		...
		{
			// Put pack the old swap chain handle into the VulkanWindow; this 
			// ensures that the old swap chain is destroyed when this error 
			// branch occurs. 
		...
		throw lut::Error("recreate_swapchain Not yet implemented!");
	}
}
...
namespace
{
	...
	{
		std::uint32_t numSurfaceFormats = 0;
		if (auto const res = vkGetPhysicalDeviceSurfaceFormatsKHR(aPhysicalDev, aSurface, &numSurfaceFormats, nullptr); VK_SUCCESS != res)
		{
			throw lut::Error("Error: unable to enumerate surface formats!");
		}

		std::vector<VkSurfaceFormatKHR> surfaceFormats(numSurfaceFormats);
		if (auto const res = vkGetPhysicalDeviceSurfaceFormatsKHR(aPhysicalDev, aSurface, &numSurfaceFormats, surfaceFormats.data()); VK_SUCCESS != res)
		{
			throw lut::Error("Error: unable to get surface format properties");
		}

		return surfaceFormats;
		//TODO: implement me!
		throw lut::Error("get_surface_formats Not yet implemented!");
	}
	...
	std::unordered_set<VkPresentModeKHR> get_present_modes(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface)
	{
		...
		// inserting results into a set and return it:
		return std::unordered_set<VkPresentModeKHR>(presentModes.begin(), presentModes.end());

		//TODO: implement me!
		throw lut::Error("get_present_modes Not yet implemented!");
	}
	...
	std::tuple<VkSwapchainKHR, VkFormat, VkExtent2D> create_swapchain(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface, VkDevice aDevice, GLFWwindow* aWindow, std::vector<std::uint32_t> const& aQueueFamilyIndices, VkSwapchainKHR aOldSwapchain)
	{
		...
		//printf("Selected image count is: %u\n", imageCount);


		//=========================================================================================
		//TODO: figure out swap extent
		//VkExtent2D extent{}; // FIXME
		// Figure out the swap extent 1
		...
		//=========================================================================================
		throw lut::Error("create_swapchain Not yet implemented!");
	}
	...
	void get_swapchain_images(VkDevice aDevice, VkSwapchainKHR aSwapchain, std::vector<VkImage>& aImages)
	{
		...
		if (auto const res = vkGetSwapchainImagesKHR(aDevice, aSwapchain, &numSwapchainImages, nullptr); VK_SUCCESS != res)
		{
			throw lut::Error("Error: unable to enumerate surface formats!");
		}
		//printf("numSwapchainImages = %u\n", numSwapchainImages);
		aImages.resize(numSwapchainImages);

		if (auto const res = vkGetSwapchainImagesKHR(aDevice, aSwapchain, &numSwapchainImages, aImages.data()); VK_SUCCESS != res)
		{
			throw lut::Error("Error: unable to get surface format properties");
		}
		...
		throw lut::Error("get_swapchain_images Not yet implemented!");
	}
	...
	void create_swapchain_image_views(VkDevice aDevice, VkFormat aSwapchainFormat, std::vector<VkImage> const& aImages, std::vector<VkImageView>& aViews)
	{
		...
		throw lut::Error("create_swapchain_image_views Not yet implemented!");
...
namespace
{
	...
	VkDevice create_device(VkPhysicalDevice aPhysicalDev, std::vector<std::uint32_t> const& aQueues, std::vector<char const*> const& aEnabledExtensions)
	{
		...
		std::printf("The supported maximum sampler anisotropy value is:  %.2f\n", maxAnisotropy);
