...
namespace
{
	...
	namespace cfg
	{
		...
		RenderMode currMode = RENDER_MODE_TEXTURE;
...
#		define SHADERDIR_ "assets/cw1/shaders/"
		...
		constexpr char const* kFragShaderPath_MPMP = SHADERDIR_ "shaderMPMP.frag.spv";

		constexpr char const* kVertShaderPath2 = SHADERDIR_ "default.vert.spv";
		constexpr char const* kFragShaderPath2 = SHADERDIR_ "default.frag.spv";

#		undef SHADERDIR_

#		define ASSERTDIR_ "assets/cw1/"
		constexpr char const* kWaveObject = ASSERTDIR_ "sponza_with_ship.obj";
		constexpr char const* kWaveTexture = ASSERTDIR_ "sponza_with_ship.mtl";
#		undef ASSERTDIR_
	...
	void record_commands(
		...
		std::vector<VkDescriptorSet> aObjectDescriptors,
		TexturedMesh& texturedMesh,
		VkPipeline,
		ColorizedMesh& untexturedMesh
	);
...
{
	// load model
	//SimpleModel kWaveModel = load_simple_wavefront_obj(cfg::kWaveObject);
	const std::vector<std::string> pathVec = getTexPath();
	...
	lut::Pipeline secondPipe = create_second_pipeline(window, renderPass.handle, pipeLayout.handle);
	...
	// Load data
	//ColorizedMesh triangleMesh = create_triangle_mesh( window, allocator ); // old
	TexturedMesh texturedMesh = handle_textured_mesh(window, allocator);
	ColorizedMesh unTexturedMesh = handle_untextured_mesh(window, allocator);
	...
	std::vector<lut::Image> myImageVec(pathVec.size());
	for (auto i = 0; i < pathVec.size(); i++)
	{
		if (pathVec[i] != "")
		{
			lut::Image tempImg;
			tempImg = lut::load_image_texture2d(pathVec[i].c_str(), window, cpool.handle, allocator);
			myImageVec[i] = (std::move(tempImg));
		}		
	}

	std::vector<lut::ImageView> myImageViewVec(pathVec.size());
	for (auto i = 0; i < pathVec.size(); i++)
	{		
		if (myImageVec[i].image) 
		{
			lut::ImageView tempView;
			tempView = lut::create_image_view_texture2d(window, myImageVec[i].image, VK_FORMAT_R8G8B8A8_SRGB);
			myImageViewVec[i] = (std::move(tempView));
		}
	}

	std::vector<VkDescriptorSet> myDescriptorsVec(pathVec.size());
	for (auto i = 0; i < pathVec.size(); i++)
	{		
		if (myImageViewVec[i].handle)
		{
			...
			textureInfo.imageView = myImageViewVec[i].handle;
	...
	while (!glfwWindowShouldClose(window.window))
	{
		...
		if (recreateSwapchain || state.changeMode)
		{
			...
			if (changes.changedSize || state.changeMode)
			{
				...
				secondPipe = create_second_pipeline(window, renderPass.handle, pipeLayout.handle);
			}
...
namespace
{
	void glfw_callback_key_press(GLFWwindow* aWindow, int aKey, int /*aScanCode*/, int aAction, int /*aModifierFlags*/)
	{
		...
		case GLFW_KEY_1:
			state->inputMap[std::size_t(EInputState::renderTEX)] = !isReleased;
			break;

		case GLFW_KEY_2:
			state->inputMap[std::size_t(EInputState::renderMPMP)] = !isReleased;
			break;
	...
	void update_user_state(UserState& aState, float aElapsedTime)
	{
		...
		if (aState.inputMap[std::size_t(EInputState::renderMPMP)])
		{
			cfg::currMode = cfg::RENDER_MODE_MIPMAP;
			aState.changeMode = true;
		}			
		if (aState.inputMap[std::size_t(EInputState::renderTEX)])
		{
			cfg::currMode = cfg::RENDER_MODE_TEXTURE;
			aState.changeMode = true;
		}
...
namespace
{
	...
	lut::Pipeline create_pipeline(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, VkPipelineLayout aPipelineLayout)
	{
		...
		if (cfg::currMode == cfg::RENDER_MODE_TEXTURE)
			frag = lut::load_shader_module(aWindow, cfg::kFragShaderPath);
		if (cfg::currMode == cfg::RENDER_MODE_MIPMAP)
			frag = lut::load_shader_module(aWindow, cfg::kFragShaderPath_MPMP);
	...
	)
	{
		...
		for (size_t i = 0; i < texturedMesh.meshCount; i++)
		{
			vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, aGraphicsLayout, 1, 1, &aObjectDescriptors[texturedMesh.texID[i]], 0, nullptr);
			VkBuffer buffers[2] = { texturedMesh.positions.buffer, texturedMesh.texcoords.buffer };
			VkDeviceSize offsets[2]{ sizeof(glm::vec3) * texturedMesh.offsetVec[i], sizeof(glm::vec2) * texturedMesh.offsetVec[i] };

			vkCmdBindVertexBuffers(aCmdBuff, 0, 2, buffers, offsets);
			vkCmdDraw(aCmdBuff, texturedMesh.vertexCount[i], 1, 0, 0);
		}

		vkCmdBindPipeline(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, secondPipe);
		//vkCmdBindDescriptorSets(aCmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, aGraphicsLayout, 0, 1, &aSceneDescriptors, 0, nullptr);
		for (size_t i = 0; i < untexturedMesh.meshCount; i++)
		{
			VkBuffer buffers[2] = { untexturedMesh.positions.buffer, untexturedMesh.colors.buffer };
			VkDeviceSize offsets[2]{ sizeof(glm::vec3) * untexturedMesh.offsetVec[i], sizeof(glm::vec3) * untexturedMesh.offsetVec[i] };

			vkCmdBindVertexBuffers(aCmdBuff, 0, 2, buffers, offsets);
			vkCmdDraw(aCmdBuff, untexturedMesh.vertexCount[i], 1, 0, 0);
		}
	...
	{
		lut::ShaderModule vert = lut::load_shader_module(aWindow, cfg::kVertShaderPath2);
		lut::ShaderModule frag = lut::load_shader_module(aWindow, cfg::kFragShaderPath2);
